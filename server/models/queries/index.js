function getAUser(req){
  return req.client.query('SELECT * FROM account WHERE email=($1);',
   [req.body.email.trim()]);
}


function getAUserById(req, user){
  return req.client.query('SELECT * FROM account WHERE user_id=($1)',
   [user.user_id]);
}
...
function getAllEntries(req){
  return req.client.query(`SELECT * FROM entry 
...
function insertEntry(req, title, content){
  ...
  values($1, $2, $3)`, [req.user.user_id, title, content]);
}


function updateEntry(req, title, content, date){
  return req.client.query(`UPDATE entry SET title=($1), content=($2), updated_at=($3)
  WHERE entry_id=($4)`, [title, content, date, req.entry.entry_id]);
}


function deleteEntry(req){
  return req.client.query('DELETE FROM entry WHERE entry_id=($1)',
  [req.entry.entry_id]);
}
